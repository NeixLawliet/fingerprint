...
class EmployeeController extends Controller
{
    public function get(Request $request, $id = null)
    {
        ...
        if ($id != null) {
            $res = Employee::getById($id, $params, $request);
        } else if (isset($params['all']) && $params['all']) {
            $res = Employee::getAllResult($params, $request);
        } else {
            $res = Employee::getPaginatedResult($params, $request);
        }
    ...
    {
        $params = $request->all();
        return Employee::createOrUpdate($params, $request->method(), $request);
    }
    ...
    {
        $params = $request->all();
        return Employee::deleteById($id, $params, $request);
    }

    public function approve(Request $request, $id)
    {
        $params = $request->all();
        return Employee::approveById($id, $params, $request);
    }
    ...
    public function datatables(Request $request)
    {
        ...
        $res = Employee::datatables($start, $limit, $order, $dir, $search, $filter);
        ...
        if (!empty($res['data'])) {
            foreach ($res['data'] as $row) {
                ...
                $nested_data['action'] .= '      <a href="javascript:void(0);" class="dropdown-item" id="edit-data" data-id="'.$row['id'].'">';
                $nested_data['action'] .= '          <i class="fas fa-edit me-2"></i> Edit';
